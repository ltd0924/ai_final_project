...
''' newPopulation = Population(pop.populationSize*2, True)

 elitismOffset = 2
 # If fittest chromosome has to be passed directly to next generation
 if elitism:
     pop.resizePop(populationSize)
 ...
 for i in range(elitismOffset, elitismOffset+pop.populationSize):
     ...
     if random.random() > crossoverRate:
         ...
         newPopulation.saveRoute(i, pop.getRoute(i-elitismOffset))

 # Performs Mutation
 for i in range(elitismOffset+pop.populationSize,newPopulation.populationSize):
     cls.mutate(newPopulation.getRoute(i))

 # Perform Selection
 newPopulation.resizePop(populationSize)
...
class GA:
    ...
    def evolvePopulation(cls, pop):
        newPopulation = Population(pop.populationSize*2, False)

        elitismOffset = 1
        # If fittest chromosome has to be passed directly to next generation
        if elitism:
            for i in range(elitismOffset):
                newPopulation.routes.append(pop.getRoute(i))
        ...
        for i in range(elitismOffset, pop.populationSize):
            ...
            # Adds child to next generation
            child = cls.crossover(parent1, parent2)
            newPopulation.routes.append(child)
            '''if random.random() > crossoverRate:
                child = cls.crossover(parent1, parent2)
                newPopulation.routes.append(child)
            else:
    ...
    def tournamentSelection (cls, pop):
        ...
        for i in range(tournamentSize):
            ...
            tournament.routes.append(pop.getRoute(randomInt))
